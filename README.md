...
> SOF teams must turn OSINT, signals metadata, captured media, and reporting into decisions **without cloud reachback**. Meldkit is an AI analytic node you *own*: it ingests, fuses, and reasons over intelligence entirely on your hardware — air-gapped if needed — with every output traceable to its sources.
...
Clone first: `git clone https://github.com/cognis-digital/meldkit && cd meldkit`
...
```bash
docker build -t meldkit .
docker run --rm meldkit --help
docker run --rm meldkit demo        # any meldkit subcommand works
```
</details>
...
Meldkit ingests keyless live feeds — **GDELT**, **ReliefWeb**, **USGS**, **GDACS**,
...
Meldkit is an **analytic aid**, not an autopilot. The default provider is
